...
contract PoolManager is IPoolManager, ProtocolFees, NoDelegateCall, ERC6909Claims {
    ...
    /// @inheritdoc IPoolManager
    function getLiquidityPool(PoolId id) external view override returns (uint128 liquidity) {
        return pools[id].liquidity;
    }

    /// @inheritdoc IPoolManager
    function getLiquidityPosition(PoolId id, address _owner, int24 tickLower, int24 tickUpper)
        external
        view
        override
        ...
        returns (int24 tick)
    {
        ...
        emit Initialize(id, key.currency0, key.currency1, key.fee, key.tickSpacing);
    }

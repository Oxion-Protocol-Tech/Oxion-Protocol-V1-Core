...
library SqrtPriceMath {
        ...
        returns (uint160)
    {
        ...
        uint256 numerator1 = uint256(liquidity) << FullMath.RESOLUTION;
        ...
        if (add) {
            uint256 quotient = (
                amount <= type(uint160).max
                    ? (amount << FullMath.RESOLUTION) / liquidity
                    : FullMath.mulDiv(amount, FullMath.Q96, liquidity)
            );
            ...
            uint256 quotient = (
                amount <= type(uint160).max
                    ? UnsafeMath.divRoundingUp(amount << FullMath.RESOLUTION, liquidity)
                    : FullMath.mulDivRoundingUp(amount, FullMath.Q96, liquidity)
            );
        ...
        returns (uint256 amount1)
    {
        ...
        return roundUp
            ? FullMath.mulDivRoundingUp(liquidity, sqrtRatioBX96 - sqrtRatioAX96, FullMath.Q96)
            : FullMath.mulDiv(liquidity, sqrtRatioBX96 - sqrtRatioAX96, FullMath.Q96);
    }
